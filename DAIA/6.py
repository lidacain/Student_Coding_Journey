...
# Проход по каждому столбцу, выполнение Random Forest регрессии и вывод результатов
for column in numeric_cols:
    try:
        # Создание объекта регрессора Random Forest
        ...
        plt.plot(data.index, y_pred, color='red', label='Random Forest регрессия')
        plt.xlabel('Дата')
        plt.ylabel(column)
        plt.title(f'Random Forest регрессия для столбца {column}')
        plt.legend()
        plt.show()
