...
# Проход по каждому столбцу, выполнение линейной регрессии и вывод результатов
for column in numeric_cols:
    ...
    plt.plot(data.index, y_pred, color='red', label='Линейная регрессия')
    plt.xlabel('Дата')
    plt.ylabel(column)
    plt.title(f'Линейная регрессия для столбца {column}')
    plt.legend()
    plt.show()
