...
def process_video_with_tracking(input_video, output_folder, known_faces, camera_name):
    ...
    # Счетчик для каждой сессии обработки видео
    session_counter = 1
        ...
        if success:
            ...
            for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
                ...
                if match_index is not None:
                    ...
                    face_id = f'{camera_name}_{frame_id}_{session_counter}_{len(known_faces) + 1}'  # создаем новый face_id
                    known_faces.append(
            ...
            # Проверка на схождение с маршрута
            known_faces_copy = known_faces.copy()  # Создаем копию списка для безопасного удаления элементов
            for known_face in known_faces_copy:
                ...
                if elapsed_time > absence_threshold:
                    ...
                    face_id = f"{camera_name}_{frame_id}_{session_counter}_{known_face['face_id']}"
                    ...
                    suspect_file_path = os.path.join(output_folder_suspects,
                                                     f"suspect_{camera_name}_{input_video.stem}_{session_counter}_{known_face['face_id']}.txt")

                    with open(suspect_file_path, 'w') as suspect_file:
                        suspect_file.write(suspect_data)

                    # Сохранение фотографии того, кто сошел с маршрута
                    suspect_photo_path = os.path.join(output_folder_suspects,
                                                      f"suspect_{camera_name}_{input_video.stem}_{session_counter}_{known_face['face_id']}.jpg")
                    cv2.imwrite(suspect_photo_path, frame)

                    print(f"Saved suspect data to {suspect_file_path}")
                    print(f"Saved suspect photo to {suspect_photo_path}")

                    # Удаляем лицо из списка известных по face_id
                    known_faces = [kf for kf in known_faces if kf['face_id'] != known_face['face_id']]
...
# Определение папок для лиц и кадров для каждой камеры
